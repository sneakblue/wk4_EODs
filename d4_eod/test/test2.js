...
describe ('SuperFan', () => {
    ...
    describe("SuperFan should have a `iGotThrownOut` method", () => {
        it('that accepts an `opposingTeam` arguement and adds one to the `gamesThrownOutOf` attribute', () => {
            let ejections = fan.gamesThrownOutOf;
            fan.iGotThrownOut("Bears");
            expect(fan.gamesThrownOutOf).to.equal(ejections + 1);
            fan.iGotThrownOut("Eagles");
            fan.iGotThrownOut("Washington Football Team");
